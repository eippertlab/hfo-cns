...
if __name__ == '__main__':
    ...
    for sheetname in ['10%', '20%', '25%', '33%', '50%']:
        ...
        for cond_name in cond_names:
            ...
            rank_df = pd.DataFrame(df_wavelet['Subject'])

            # loop through each score column and rank the values
            for col in df_wavelet.columns[1:]:
                rank_df[col] = df_wavelet[col].rank(method='dense', ascending=True)
            print(rank_df)
            print(f'{cond_name}, {sheetname}')
            print(rank_df[['Spinal', 'Subcortical', 'Cortical']].corr())
            rank_df.dropna(inplace=True)  # Only want this for image
            no_kept = len(rank_df.index)
            # print(df_wavelet)

            fig, ax = plt.subplots(1, figsize=(16, 10))
            rank_df = pd.melt(rank_df, id_vars=['Subject'], value_vars=[f'Spinal', f'Subcortical', f'Cortical'],
                              var_name='CNS Level', value_name='Rank')  # Change to long format

            sns.pointplot(data=rank_df,
                        x='CNS Level', y='Rank', hue='Subject', palette=sns.color_palette(['gray'], no_kept),
                        ax=ax, legend=False)
            ...
            plt.savefig(figure_path + f'CrossCNSRank_PeaksTroughs_{cond_name}_{sheetname}.png')
            # plt.savefig(
            #     figure_path + f'CrossCNSRank_PeaksTroughs_{cond_name}_{sheetname}.pdf',
