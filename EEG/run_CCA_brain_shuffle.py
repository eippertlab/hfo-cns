...
def run_CCA_shuffle(subject, condition, srmr_nr, freq_band, sfreq):
    ...
    save_path = "/data/pt_02718/tmp_data/cca_eeg_shuffle/" + subject_id + "/"
    ...
    # # Extract relevant events related to our trigger
    # relevant_events = [list(event) for event in events if event[2] == event_ids[trigger_name]]
    # # Get radom number of samples to add to each
    # samp = random.choices(np.arange(100, 500), k=len(relevant_events)) # Will give us random numbers in ms
    # res = [int((x/1000)*sfreq) for x in samp]  # Convert to seconds and then samples
    # # Add this random number of samples to each event trigger time in samples
    # relevant_events_samples = []
    # for i, shift in enumerate(res):
    #     relevant_events_samples.append([int(relevant_events[i][0]+shift), relevant_events[i][1], relevant_events[i][2]])
    ...
    if cond_name == 'median':
        ...
        if freq_band == 'sigma':
            window_times = [-50/1000, -40/1000]
        sep_latency = 20
    elif cond_name == 'tibial':
        epochs = epochs.pick_channels(eeg_chans, ordered=True)
        if freq_band == 'sigma':
            window_times = [-55/1000, -35/1000]
        sep_latency = 40
    else:
    ...
    ################################ Plotting Graphs #######################################
    figure_path_spatial = f'/data/p_02718/Images/CCA_eeg_shuffle/ComponentIsopotentialPlots/{subject_id}/'
    ...
    if plot_graphs:
        ...
        figure_path_time = f'/data/p_02718/Images/CCA_eeg_shuffle/ComponentTimePlots/{subject_id}/'
        ...
        figure_path = f'/data/p_02718/Images/CCA_eeg_shuffle/ComponentPlots/{subject_id}/'
