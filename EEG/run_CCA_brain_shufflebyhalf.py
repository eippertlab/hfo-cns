# Script to actually run CCA with random training windows on the data
...
def run_CCA_shufflebyhalf(subject, condition, srmr_nr, freq_band, sfreq):
    ...
    save_path = "/data/pt_02718/tmp_data/cca_eeg_shufflebyhalf/" + subject_id + "/"
    ...
    # Want to edit specific event times to shift by random amount
    # Jitter is 736 ms, I'm shifting by a random amount within 50ms of this number
    events_of_interest = [event for event in events if event[2] == event_id_dict[trigger_name]]
    shifted_events = []
    for i in np.arange(np.shape(events_of_interest)[0]):
        shift = random.randint(int(round(0.318 * 5000)), int(round(0.418 * 5000)))
        shifted_events.append([events_of_interest[i][0] + shift, events_of_interest[i][1], events_of_interest[i][2]])

    epochs_before = mne.Epochs(raw, events, event_id=event_id_dict, tmin=iv_epoch[0], tmax=iv_epoch[1] - 1 / 1000,
                        baseline=tuple(iv_baseline), preload=True, reject_by_annotation=True)
    epochs = mne.Epochs(raw, shifted_events, event_id=event_id_dict, tmin=iv_epoch[0], tmax=iv_epoch[1]-1/1000,
                        baseline=tuple(iv_baseline), preload=True, reject_by_annotation=True)
    ...
    ################################ Plotting Graphs #######################################
    figure_path_spatial = f'/data/p_02718/Images/CCA_eeg_shufflebyhalf/ComponentIsopotentialPlots/{subject_id}/'
    ...
    if plot_graphs:
        ...
        figure_path_time = f'/data/p_02718/Images/CCA_eeg_shufflebyhalf/ComponentTimePlots/{subject_id}/'
        ...
        figure_path = f'/data/p_02718/Images/CCA_eeg_shufflebyhalf/ComponentPlots/{subject_id}/'
