    ...
    for freq_band in freq_bands:
        for condition in conditions:
            ...
            for data_type in ['Thalamic_CCA', 'Cortical_CCA']:
                ...
                # Plot Time Course
                #################################################################################################
                ax1.plot(evoked.times, grand_average[0, :], label=f'{data_type}, n={len(evoked_list)}')
                ax1.fill_between(evoked.times, lower, upper, alpha=0.3)
                ax1.axvline(sep_latency, color='red')

            ax1.set_xlabel('Time (s)')
            ax1.set_title(f'Grand Average Envelope')
            ax1.set_ylabel('Amplitude After CCA')
            if cond_name in median_names:
