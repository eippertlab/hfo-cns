...
if __name__ == '__main__':
    ...
    for condition in conditions:
        for subject in subjects:
            ...
            save_path = "/data/p_02718/Images/ESG/GoodOnly/"
            ...
            evoked_good = epochs_good.average().pick_channels([channel])
            evoked = epochs.average().pick_channels([channel])

            plt.figure()
            plt.plot(epochs.times, evoked.get_data().reshape(-1), label='All')
            plt.plot(epochs.times, evoked_good.get_data().reshape(-1), label='Good')
            ...
            plt.title(f'{subject_id}_{cond_name}_{freq_band}')
            plt.legend()
            plt.savefig(save_path+f'{subject_id}_{cond_name}_{freq_band}.png')
            plt.close()
            # plt.show()
