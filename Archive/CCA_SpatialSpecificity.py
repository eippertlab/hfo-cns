...
if __name__ == '__main__':
    ...
    average = [False, False, True, True]

    anterior = [False, True, False, True]  # If true, use the anteriorly rereferenced data
    # Otherwise use the normal data

    for use_average, use_anterior in zip(average, anterior):
        for freq_band in freq_bands:
            for condition in conditions:
                ...
                for subject in subjects:
                    ...
                    if use_anterior:
                        input_path_incorrect = "/data/pt_02718/tmp_data/cca_opposite_anterior/" + subject_id + "/"
                    else:
                ...
                if use_average:
                    if use_anterior:
                        plt.savefig(figure_path + f'Average_GA_Envelope_{freq_band}_{cond_name}_n={len(evoked_list)}_ant')
                    ...
                    if use_anterior:
                        plt.savefig(figure_path + f'Cor1_GA_Envelope_{freq_band}_{cond_name}_n={len(evoked_list)}_ant')
