...
if __name__ == '__main__':
    ...
    figure_path = '/data/p_02718/Images/Reconstructed_CCA_eeg/SingleSubject/'
    ...
    for freq_band in freq_bands:
        for condition in conditions:
            for subject in subjects:
                ...
                ax.set_title(f'Time Course, Subject {subject}')
                if cond_name == 'median':
                    ax.set_xlim([0.00, 0.05])
                else:
                    ax.set_xlim([0.00, 0.07])
                plt.savefig(figure_path + f'{subject}_Time_{freq_band}_{cond_name}')
                ...
                evoked.plot_topomap(times=times, average=None, ch_type=None, scalings=None, proj=False,
                                    ...
                                    time_format=None, title=f'Spatial Pattern, Subject {subject}',
                                    nrows=1, ncols='auto', show=True)
                plt.savefig(figure_path + f'{subject}_Spatial_{freq_band}_{cond_name}')
