...
from Common_Functions.calculate_snr_lowfreq_ccapolarity import calculate_snr_lowfreqcca_polarity
...
if __name__ == '__main__':
    ...
    if srmr_nr == 1:
        ...
        component_fname = '/data/pt_02718/tmp_data/Components_EEG_Updated_LF_autoflip.xlsx'
        visibility_fname = '/data/pt_02718/tmp_data/Visibility_Updated_LF_autoflip.xlsx'
        figure_path = '/data/p_02718/Images/CCA_eeg_low/SNR&EnvelopePeak_autoflip/'
        ...
        component_fname = '/data/pt_02718/tmp_data_2/Components_EEG_Updated_LF_autoflip.xlsx'
        visibility_fname = '/data/pt_02718/tmp_data_2/Visibility_Updated_LF_autoflip.xlsx'
        figure_path = '/data/p_02718/Images_2/CCA_eeg_low/SNR&EnvelopePeak_autoflip/'
        os.makedirs(figure_path, exist_ok=True)
    ...
    for condition in conditions:
        ...
        for subject in subjects:
            ...
            if srmr_nr == 1:
                ...
                input_path = "/data/pt_02718/tmp_data/cca_eeg_low/" + subject_id + "/"
            elif srmr_nr == 2:
                fname = f'noStimart_sr5000_{cond_name}_withqrs_eeg.fif'
                input_path = "/data/pt_02718/tmp_data_2/cca_eeg_low/" + subject_id + "/"
            ...
            for c in np.arange(0, n_components):  # Loop through all components
                ...
                # Automatically flip if correct polarity not observed
                value = evoked.copy().crop(tmin=sep_latency, tmax=sep_latency).get_data()[0][0]

                if cond_name in ['median', 'med_mixed']:
                    if value > 0:
                        evoked.apply_function(invert, picks=channel)
                        flip_comp[c] = 'T'
                else:
                    if value < 0:
                        evoked.apply_function(invert, picks=channel)
                ...
                # Get SNR of LF components
                noise_window = [-100/1000, -10/1000]
                snr = calculate_snr_lowfreqcca_polarity(evoked.copy(), noise_window, signal_window, sep_latency, 'cortical', cond_name)
                snr_comp.append(snr)
                ...
                if plot_image:
                    ...
                    ax[c].axvline(x=sep_latency+signal_window, color='r', linewidth='1')
                    ax[c].set_title(f'Component {c+1}, SNR: {snr:.2f}, FLip:{flip_comp[c]}')
                    if cond_name in ['median', 'med_mixed']:
